...
pub use crate::gimli_decrypt::GimliAeadDecryptIter;
pub use crate::gimli_encrypt::GimliAeadEncryptIter;
use std::cmp::min;
use std::io;
...
pub fn gimli_hash(
  mut input: impl Iterator<Item = Result<u8, io::Error>>,
  mut input_byte_len: u64,
  mut output_byte_len: u64,
) -> Vec<u8> {
...
mod tests {
  ...
  fn hash_test() {
    ...
    for vec in hash_vectors.iter() {
      ...
      assert_eq!(
        vec.1,
        gimli_hash(
          vec.0.to_string().into_bytes().into_iter().map(|x| Ok(x)),
          input_len,
          vec.2
        )
        .iter()
  ...
  fn test_cipher() {
    ...
    for vec in cipher_vectors.iter() {
      ...
      let ge_iter = GimliAeadEncryptIter::new(key, nonce, pt_len, Box::new(pt.clone()), assoc_d);
      let result: Vec<u8> = ge_iter.collect();
      assert_eq!(vec.2, result);

      let gd_iter = GimliAeadDecryptIter::new(key, nonce, ct_len, Box::new(ct.clone()), assoc_d);
